// ─── PayOnUs Webhook Simulator ────────────────────────────────────────────────
// Run with: npx tsx scripts/test-payonus-fund.ts <email>
// Example:  npx tsx scripts/test-payonus-fund.ts you@example.com
// Looks up user by email, then fires webhook for their latest pending top-up
...
// const WEBHOOK_URL   = "https://freya-k67i5up1h-jamins-projects-863acee5.vercel.app/api/webhooks/payonus";
const VERIFICATION_KEY = "freya_payonus_webhook_2026";

const ONUS_REFERENCE = "ONUS-NUB-TEST-" + Date.now();
...
async function main() {
  ...
  if (!email) {
    ...
    console.error("Usage: npx tsx scripts/test-payonus-fund.ts <email>");
    process.exit(1);
  }

  console.log("─── PayOnUs Webhook Simulator ───────────────────────────");
  ...
  // 2. Fetch latest pending wallet topup for this user
  ...
  if (fetchError || !transaction) {
    console.error("❌ No pending wallet top-up found for this user");
    console.error("→ Make sure you've initiated a top-up in the app first");
    process.exit(1);
  }
  ...
  if (res.status === 200) {
    ...
    console.log("→ Check your wallet balance — should show ₦" + AMOUNT.toLocaleString());
  } else {
