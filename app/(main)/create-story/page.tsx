...
export default function CreateStoryPage() {
  ...
  const handleClose = () => nav.back();

  const handleUploadStart = (job: UploadJob) => {
    startUpload(job);
    nav.push(viewer?.username ? `/${viewer.username}` : "/");
  };

  const state = useStoryUploadState({ onClose: handleClose, onUploadStart: handleUploadStart });
