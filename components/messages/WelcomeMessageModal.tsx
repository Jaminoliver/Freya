...
export function WelcomeMessageModal({ onClose, onSave }: Props) {
  ...
  // ─── Load existing welcome message on mount ─────────────────────────────────
    ...
    async function loadExisting() {
      try {
        ...
        if (data.message) {
          ...
          setPpvPrice(data.message.ppv_price ? String(data.message.ppv_price) : "");
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (isPpv && ppvPrice) {
        formData.append("ppv_price", ppvPrice);
      }

      // Append existing media IDs to keep
