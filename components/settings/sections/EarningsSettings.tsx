  ...
  return (
    <div style={{ fontFamily: "'Inter', sans-serif" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "16px" }}>
        ...
        <h2 style={{ fontSize: "16px", fontWeight: 700, color: "#F1F5F9", margin: 0 }}>Earnings</h2>
      </div>
...
function HistoryTabContent() {
  ...
  const fetchHistory = useCallback(async (type: string) => {
    ...
    try {
      ...
      const res = await fetch(`/api/earnings/history?type=${param}`);
  ...
  return (
    <div>
      ...
      {loading ? (
        <p style={{ fontSize: "12px", color: "#6B6B8A", fontFamily: "'Inter', sans-serif" }}>Loading...</p>
      ) : history.length === 0 ? (
        ...
        <div style={{ display: "flex", flexDirection: "column" }}>
          {history.map((item) => (
            <div key={item.id} style={{ display: "flex", alignItems: "center", gap: "12px", padding: "12px 0", borderBottom: "1px solid #1A1A2E" }}>
              <div style={{ width: "32px", height: "32px", borderRadius: "50%", flexShrink: 0, background: "linear-gradient(135deg, #8B5CF6, #EC4899)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "12px", fontWeight: 700, color: "#fff" }}>
                {item.fan.charAt(0)}
              </div>
