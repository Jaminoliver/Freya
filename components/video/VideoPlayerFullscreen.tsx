...
export function VideoPlayerFullscreen({
  ...
  useEffect(() => {
    if (username) router.prefetch(`/${username}`);
  }, [username, router]);
  ...
  const cached = postSyncStore.get(postIdStr);
  const [likeCount, setLikeCount] = useState(cached?.like_count ?? initialLikeCount);
  const [liked, setLiked] = useState(cached?.liked ?? initialLiked);
  const [commentCount, setCommentCount] = useState(cached?.comment_count ?? initialCommentCount);
  ...
  const videoSrc = bunnyVideoId
    ? `https://${STREAM_CDN}/${bunnyVideoId}/playlist.m3u8`
    : null;

  const videoRatio = (() => {
    if (aspectRatio != null && aspectRatio > 0) return aspectRatio;
  ...
  useEffect(() => {
    const unsub = postSyncStore.subscribe((event) => {
      if (event.postId !== postIdStr) return;
  ...
  const handleFollowToggle = async (e: React.MouseEvent) => {
    ...
    try {
      if (isFollowing) {
        ...
        onFollowChange?.(creatorId, false);
      } else {
  ...
  const handleLike = async (e: React.MouseEvent) => {
    ...
    postSyncStore.emit({ postId: postIdStr, liked: newLiked, like_count: newCount });
    try {
      const res = await fetch(`/api/posts/${postId}/like`, { method: "POST" });
      const json = await res.json();
      setLiked(json.liked);
      setLikeCount(json.like_count);
      postSyncStore.emit({ postId: postIdStr, liked: json.liked, like_count: json.like_count });
    } catch {
      setLiked(!newLiked);
      setLikeCount(likeCount);
      postSyncStore.emit({ postId: postIdStr, liked: !newLiked, like_count: likeCount });
    }
  };

  const handleAddComment = async (pid: string, text: string, gif_url?: string, parent_comment_id?: string | number, reply_to_username?: string | null, reply_to_id?: string | number | null) => {
    ...
    if (res.ok && json.comment) {
      if (!parent_comment_id) {
        ...
        postSyncStore.emit({ postId: postIdStr, liked, like_count: likeCount, comment_count: newCount });
    ...
    <>
      ...
      >
        ...
        >
          ...
          {/* Mute — top right */}
          <button
            onClick={(e) => { e.stopPropagation(); onMuteChange(!isMuted); }}
            style={{
          ...
          >
            ...
            <div style={{ position: "relative", marginBottom: 4 }}>
              ...
              >
                {avatarError || !avatarUrl ? (
                  ...
                  <img src={avatarUrl} alt={name} onError={() => setAvatarError(true)} style={{ width: "100%", height: "100%", objectFit: "cover", display: "block" }} />
                )}
              </button>
              {!isFollowing && (
                <button
            ...
            {/* Subscribers */}
            <button className="vpfs-action-btn" onClick={handleProfileClick}>
              ...
              <span style={{ color: "#F5C842" }}>{formatCount(subscriberCount)}</span>
            </button>

            {/* Likes */}
            <button className="vpfs-action-btn" onClick={handleLike}>
            ...
            {/* Comments */}
            <button className="vpfs-action-btn" onClick={(e) => { e.stopPropagation(); if (!viewerUserId) { openAuthModal(); return; } setCommentsOpen(true); }}>
          ...
          >
            <div onClick={handleProfileClick} style={{ cursor: "pointer", marginBottom: caption ? 6 : 0 }}>
            ...
            {caption && <CaptionExpander caption={caption} />}
          </div>
