    ...
    <>
        ...
        <div className="md:hidden" style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 100, backgroundColor: "#13131F", borderBottom: "1px solid #1F1F2A", height: "56px", fontFamily: "'Inter', sans-serif" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "10px", padding: "0 12px", height: "100%", opacity: searchOpen ? 1 : 0, transform: searchOpen ? "translateY(0)" : "translateY(-10px)", transition: "all 0.2s ease", pointerEvents: searchOpen ? "auto" : "none", position: "absolute", inset: 0 }}>
            ...
            <div style={{ position: "relative", flex: 1 }}>
              <input ref={inputRef} type="text" placeholder="Search creators..." value={query} onChange={(e) => setQuery(e.target.value)} style={{ width: "100%", borderRadius: "10px", padding: "10px 36px 10px 14px", fontSize: "14px", outline: "none", backgroundColor: "#1E1E2E", border: "1.5px solid #2A2A3D", color: "#FFFFFF", boxSizing: "border-box", fontFamily: "'Inter', sans-serif" }} />
              {query && <button onClick={() => setQuery("")} style={{ position: "absolute", right: "10px", top: "50%", transform: "translateY(-50%)", background: "none", border: "none", cursor: "pointer", color: "#6B6B8A", display: "flex", padding: 0 }}><X size={14} /></button>}
            </div>
          </div>
        </div>
      )}
      ...
      {isDashboard && searchOpen && (
        <div className="md:hidden" style={{ position: "fixed", top: "56px", left: 0, right: 0, bottom: "64px", backgroundColor: "#0A0A0F", zIndex: 99, overflowY: "auto", animation: "fadeIn 0.2s ease", fontFamily: "'Inter', sans-serif" }}>
          ...
          {!searching && query.trim() && results.length === 0 && <div style={{ padding: "40px 20px", textAlign: "center", color: "#6B6B8A", fontSize: "14px" }}>No creators found for "{query}"</div>}
          {!searching && results.map((creator) => (
            <div key={creator.id} onClick={() => { pendingClose.current = true; router.push(`/${creator.username}`); }} style={{ display: "flex", alignItems: "center", gap: "12px", padding: "14px 20px", borderBottom: "1px solid #1F1F2A", cursor: "pointer" }} onMouseEnter={(e) => (e.currentTarget.style.backgroundColor = "#1E1E2E")} onMouseLeave={(e) => (e.currentTarget.style.backgroundColor = "transparent")}>
...
function MobileSuggestedCreators({ onSelect }: { onSelect: (username: string) => void }) {
  ...
  useEffect(() => {
    const load = async () => {
      const supabase = createClient();
      const { data } = await supabase.from("profiles").select("id, username, display_name, avatar_url, is_verified").eq("role", "creator").limit(8);
  ...
  return (
    <div>
      <p style={{ padding: "16px 20px 8px", margin: 0, fontSize: "13px", fontWeight: 600, color: "#6B6B8A", textTransform: "uppercase", letterSpacing: "0.06em" }}>Suggested Creators</p>
      {creators.map((creator) => (
        <div key={creator.id} onClick={() => onSelect(creator.username)} style={{ display: "flex", alignItems: "center", gap: "12px", padding: "14px 20px", borderBottom: "1px solid #1F1F2A", cursor: "pointer" }} onMouseEnter={(e) => (e.currentTarget.style.backgroundColor = "#1E1E2E")} onMouseLeave={(e) => (e.currentTarget.style.backgroundColor = "transparent")}>
          <Avatar src={creator.avatar_url ?? undefined} alt={creator.display_name || creator.username} size="md" showRing />
          <div>
            <div style={{ display: "flex", alignItems: "center", gap: "5px" }}>
              <span style={{ fontSize: "15px", fontWeight: 600, color: "#FFFFFF" }}>{creator.display_name || creator.username}</span>
