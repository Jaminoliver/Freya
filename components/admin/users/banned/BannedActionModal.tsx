...
import { X, ShieldOff, ShieldCheck, Clock, Ban } from "lucide-react";
import type { BannedUser } from "./BannedTable";

export type BannedModalAction = "lift" | "reduce" | "extend" | "reban";

interface Props {
  action:     BannedModalAction;
  user:       BannedUser;
  onConfirm:  () => void;
  onCancel:   () => void;
}

const CONFIG: Record<BannedModalAction, {
  icon:    React.ReactNode;
  title:   string;
  message: (name: string) => string;
  confirm: string;
  style:   "safe" | "danger" | "warning" | "info";
}> = {
  lift: {
    icon:    <ShieldOff size={20} />,
    title:   "Lift Ban",
    message: (n) => `${n}'s ban will be lifted immediately. They will regain full access to the platform.`,
    confirm: "Lift Ban",
    style:   "safe",
  },
  reduce: {
    icon:    <Clock size={20} />,
    title:   "Reduce Penalty",
    message: (n) => `${n}'s penalty will be reduced. Their suspension duration will be shortened based on the selected period.`,
    confirm: "Reduce Penalty",
    style:   "info",
  },
  extend: {
    icon:    <Ban size={20} />,
    title:   "Extend Ban",
    message: (n) => `${n}'s ban will be extended. This action will be logged in the audit trail.`,
    confirm: "Extend Ban",
    style:   "warning",
  },
  reban: {
    ...
    title:   "Re-ban User",
    message: (n) => `${n} will be permanently banned from the platform. This action will be logged and the user will be notified.`,
    confirm: "Re-ban",
    style:   "danger",
  },
};
...
export default function BannedActionModal({ action, user, onConfirm, onCancel }: Props) {
  ...
  return (
    <>
      ...
      <div className="modal" role="dialog" aria-modal="true">
        ...
        <div className="user-pill">
          ...
          <div>
            ...
            <div className="pill-meta">{user.email}</div>
          </div>
        </div>
      ...
      <style jsx>{`
        ...
        .modal-title {
          font-size: 17px; font-weight: 700; color: #0f0e1a;
          margin-bottom: 8px; letter-spacing: -0.3px;
        }
        .modal-message {
          font-size: 13.5px; color: #6b6a80; line-height: 1.55; margin-bottom: 18px;
        }
        .user-pill {
        ...
        .pill-meta { font-size: 12px; color: #9b9aaa; margin-top: 1px; }
